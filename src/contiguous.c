...
void contiguous_query(Synmap * syn, FILE * intfile, bool pblock)
{
  ...
  while (fgets(line, length, intfile) && !feof(intfile)) {
    ...
    for(; cslist != NULL; cslist = cslist->next){
        ...
        bound_results[LO] = get_si_bound(bounds[LO], set_bounds, blk_bounds, LO, inverted);
        bound_results[HI] = get_si_bound(bounds[HI], set_bounds, blk_bounds, HI, inverted);
    ...
    bool inverted)
{
    ...
    // In this case, the hi and lo Contiguous nodes will be the same
        ...
        Block * nearest = closest_block(con, q, d);
        ...
        if(nearest != NULL){
            ...
            // TODO: It is absurd that I have to pass syn everywhere, I should
            // add a pointer in each Block to each mirror block.
            if(REL_GT(q, nearest->over->pos[!d], d)){
