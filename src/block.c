...
void merge_block_a_into_b_side_(Block * a, Block * b){
    for(int i = 0; i < 2; i++){
        if(REL_GT(a->pos[i], b->pos[i], i)){
            ...
            if(a->parent->cor[i] == a)
                a->parent->cor[i] = b;
            if(a->parent->cor[i+2] == a)
                a->parent->cor[i+2] = b;
        } else {
            if(a->cor[i] != NULL)
                a->cor[i]->cor[!i] = a->cor[!i];
            if(a->cor[i + 2] != NULL)
                a->cor[i + 2]->cor[!i + 2] = a->cor[!i + 2];
            if(a->parent->cor[i] == a){
                if(a->cor[i] == NULL){
                    a->parent->cor[i] = a->cor[!i];
                } else {
                    a->parent->cor[i] = a->cor[i];
                }
            }
...
void merge_block_a_into_b(Block * a, Block * b){
    ...
    merge_block_a_into_b_side_(a, b);
    merge_block_a_into_b_side_(a->over, b->over);
