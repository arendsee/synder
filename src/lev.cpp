...
void convert_seqname(FILE * synfile, FILE * intfile, int swap)
{
  ...
  while (fgets(nline, length, intfile) && !feof(intfile)) {
    ...
    TOLC(seqname);
    current = root;
    // Find min lev. distance from given name, conert if distance = 0
    ...
    // If no perfect match, then prompt asking for proper match
    if (min != 0) {
